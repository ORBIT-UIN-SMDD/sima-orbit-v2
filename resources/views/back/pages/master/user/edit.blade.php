                        ...
                        <div class="card-body pt-0">
                            <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" name="role_admin" value="1"
                                    @checked($user->hasRole('super-admin')) id="roleAdmin" />
                            ...
                            <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" name="role_keuangan" value="1"
                                    @checked($user->hasRole('keuangan')) id="roleKeuangan" />
                            ...
                            <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" name="role_editor" value="1"
                                    @checked($user->hasRole('editor')) id="roleEditor" />
                            ...
                            <div class="form-check mt-3">
                                <input class="form-check-input" type="checkbox" name="role_humas" value="1"
                                    @checked($user->hasRole('humas')) id="roleHumas" />
                            ...
                            <div class="mb-5">
                                ...
                                <div class="form-check form-switch form-check-custom form-check-solid">
                                    <input class="form-check-input" type="checkbox" name="is_active" value="1"
                                        @checked(old('is_active', $user->is_active)) id="isActive" />
                            ...
                            @foreach ($permissions as $permission)
                                <div class="form-check mt-3">
                                    <input class="form-check-input" type="checkbox" name="permissions[]"
                                        ...
                                        @checked($userPermissions->contains('name', $permission->name))
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body pt-0">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-5 fv-row">
                                    ...
                                    <input type="text" name="place_of_birth" class="form-control mb-2"
                                        placeholder="Tempat Lahir" value="{{ old('place_of_birth', $user->place_of_birth) }}" />
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-5 fv-row">
                                    ...
                                    <select name="blood_type" class="form-select form-select-solid">
                                        <option value="">-- Pilih --</option>
                                        <option value="A+" @selected(old('blood_type', $user->blood_type) == 'A+')>A+</option>
                                        <option value="A-" @selected(old('blood_type', $user->blood_type) == 'A-')>A-</option>
                                        <option value="B+" @selected(old('blood_type', $user->blood_type) == 'B+')>B+</option>
                                        <option value="B-" @selected(old('blood_type', $user->blood_type) == 'B-')>B-</option>
                                        <option value="AB+" @selected(old('blood_type', $user->blood_type) == 'AB+')>AB+</option>
                                        <option value="AB-" @selected(old('blood_type', $user->blood_type) == 'AB-')>AB-</option>
                                        <option value="O+" @selected(old('blood_type', $user->blood_type) == 'O+')>O+</option>
                                        <option value="O-" @selected(old('blood_type', $user->blood_type) == 'O-')>O-</option>
                                    </select>
                                    @error('blood_type')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                            ...
                            <div class="mb-5 fv-row">
                                ...
                                <textarea name="bio" class="form-control mb-2" rows="3" placeholder="Bio singkat">{{ old('bio', $user->bio) }}</textarea>
                                @error('bio')
                                    <div class="text-danger fs-7">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="row mb-5">
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="text" name="latitude" id="latitude" class="form-control mb-2"
                                        placeholder="Latitude" value="{{ old('latitude', $user->latitude) }}" readonly />
                                    @error('latitude')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-md-6 fv-row">
                                    ...
                                    <input type="text" name="longitude" id="longitude" class="form-control mb-2"
                                        placeholder="Longitude" value="{{ old('longitude', $user->longitude) }}" readonly />
                                    @error('longitude')
                                        <div class="text-danger fs-7">{{ $message }}</div>
                                    @enderror
                                </div>
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Koordinat dari user atau default (Bukittinggi)
            var userLat = {{ old('latitude', $user->latitude ?? -0.3055) }};
            var userLng = {{ old('longitude', $user->longitude ?? 100.3692) }};
            var hasLocation = {{ ($user->latitude && $user->longitude) || old('latitude') ? 'true' : 'false' }};

            var map = L.map('locationMap').setView([userLat, userLng], hasLocation ? 15 : 13);
            ...
            // Jika user sudah punya lokasi, tampilkan marker
            if (hasLocation) {
