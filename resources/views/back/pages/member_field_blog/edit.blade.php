...
@section('content')
    <div id="kt_content_container" class="container-xxl">
        <form id="kt_ecommerce_edit_blog_form" class="form d-flex flex-column flex-lg-row"
            action="{{ route('back.member-field-blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            <div class="rounded-circle w-15px h-15px
                                ...
                                @elseif($blog->status == 'draft') bg-primary
                                @else bg-danger @endif"
                                id="kt_ecommerce_edit_blog_status">
                            </div>
                        </div>
                    </div>
                    <div class="card-body pt-0">
                        <select name="status" class="form-select mb-2" data-control="select2" data-hide-search="true"
                            data-placeholder="Pilih Status" id="kt_ecommerce_edit_blog_status_select" required>
                            <option></option>
                            ...
                            <select name="member_field_id" class="form-select mb-2" data-control="select2"
                                ...
                                @foreach ($memberFields as $field)
                                    <option value="{{ $field->id }}"
                                        {{ old('member_field_id', $blog->member_field_id) == $field->id ? 'selected' : '' }}>
                                        {{ $field->name }}</option>
                                @endforeach
                            </select>
...
@section('scripts')
    <script>
        ...
        // Initialize tagify with existing keywords
        var existingKeywords = "{{ old('meta_keywords', $blog->meta_keywords) }}";
        var tagifyInput = document.querySelector("#keyword_tagify");
        ...
        // Load existing keywords as tags
        if (existingKeywords) {
            var keywordsArray = existingKeywords.split(',').map(function(keyword) {
                return keyword.trim();
        ...
        const statusSelect = document.querySelector('#kt_ecommerce_edit_blog_status_select');
        const statusIndicator = document.querySelector('#kt_ecommerce_edit_blog_status');
